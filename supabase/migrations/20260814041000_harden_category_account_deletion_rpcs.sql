-- Keep privileged destructive logic outside the exposed API schema. Public
-- functions are security-invoker wrappers and remain the only RPC endpoints.

create or replace function private.delete_category_with_replacement(
...
set search_path = ''
as $$
  select private.delete_category_with_replacement(
    target_category_id,
    replacement_category_id
  );
$$;

create or replace function private.delete_archived_account(
...
set search_path = ''
as $$
  select private.delete_archived_account(target_account_id);
$$;

revoke all on function private.delete_category_with_replacement(uuid, uuid)
from public, anon, authenticated;
grant execute on function private.delete_category_with_replacement(uuid, uuid)
to authenticated;
...
revoke all on function private.delete_archived_account(uuid)
from public, anon, authenticated;
grant execute on function private.delete_archived_account(uuid)
to authenticated;
...
comment on function private.delete_category_with_replacement(uuid, uuid) is
  'Owner-scoped category deletion with atomic financial-reference reassignment.';
comment on function public.delete_category_with_replacement(uuid, uuid) is
  'Security-invoker RPC wrapper for owner-scoped category deletion.';
comment on function private.delete_archived_account(uuid) is
  'Owner-scoped permanent deletion of an archived account and dependent history.';
comment on function public.delete_archived_account(uuid) is
  'Security-invoker RPC wrapper for owner-scoped archived-account deletion.';
