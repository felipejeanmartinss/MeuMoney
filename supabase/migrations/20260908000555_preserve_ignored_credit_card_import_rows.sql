-- Credit-card purchase imports replaced the generic refresh wrapper that kept
-- explicitly ignored rows stable. Preserve those choices around the current,
-- card-aware refresh routine so later classifications cannot reopen them.
alter function private.refresh_import_job(uuid, uuid)
rename to refresh_import_job_before_persistent_credit_card_ignored_rows;
...
revoke all on function private.refresh_import_job_before_persistent_credit_card_ignored_rows(
  uuid,
  uuid
...
comment on function private.refresh_import_job(uuid, uuid) is
  'Recalculates account and credit-card imports while preserving every row explicitly ignored by its owner.';
