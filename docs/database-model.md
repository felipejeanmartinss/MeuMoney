...
`public.account_balances` é uma view com `security_invoker`. Ela deriva `current_balance_minor` do saldo inicial, dos lançamentos e das movimentações de transferência que estejam ativos, realizados e datados até o dia corrente no fuso de São Paulo. O saldo atual não é duplicado em uma coluna mutável; a projeção futura é calculada pelo domínio a partir do extrato completo.
