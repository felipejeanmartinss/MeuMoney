...
- Um lançamento novo ou manual editado pode encaminhar seus dados validados ao cadastro de Contas a Pagar. O lançamento é salvo primeiro; a próxima ocorrência sugerida é mensal e posterior ao lançamento, enquanto frequência, término e data continuam sujeitos à confirmação explícita na etapa seguinte.
...
- O limite comprometido soma todas as parcelas registradas ainda pendentes ou faturadas, mesmo quando o mês futuro ainda não tem fatura. Assinaturas ativas completam os meses ainda não materializados até a última fatura cadastrada, sem recriar competências já pagas. Créditos e estornos reduzem o comprometido. Na visão de Contas, “Próximas faturas” soma somente a próxima fatura exibida de cada cartão pelo valor restante (`total - pago`), por moeda; não inclui as faturas posteriores. O limite disponível é `limite total - comprometido` e não é artificialmente liberado por transferências sem baixa das parcelas.
...
- Ao registrar um lançamento realizado na mesma conta, categoria e natureza, com descrição semelhante, valor próximo e até 14 dias de distância de uma previsão pendente ou da próxima ocorrência de uma regra ativa, o formulário pede confirmação na conta e na página de novo lançamento. Descrições curtas como “TV” também são comparadas. Mesmo com descrições distintas, uma diferença pequena de valor e até sete dias de data pode gerar a sugestão para revisão humana. “É o mesmo” realiza a previsão existente ou gera e realiza atomicamente a próxima ocorrência, sem criar outro lançamento; “Não, criar separado” salva a despesa e mantém a recorrência intacta.
...
- Copiar o mês anterior mantém contexto e a moeda preferencial, lê os valores persistidos do mês anterior e atualiza ou cria as células correspondentes no destino. A operação é idempotente.
- A interface usa exclusivamente a moeda preferencial do perfil e exibe valores consolidados sem casas decimais; categorias e subcategorias são ordenadas pelo total planejado em ordem decrescente.
...
- Todo indicador consolidado é exibido na moeda preferencial do perfil. Valores em outras moedas são convertidos com a taxa implícita da transferência cambial mais recente disponível em cada mês; valores atuais usam a taxa mais recente até a data corrente.
...
- Na importação destinada a cartão, cada compra aceita diretamente o identificador de uma categoria ativa de despesa; o seletor não adiciona o prefixo usado pelo classificador misto de contas e transferências.
...
- Os tipos de relatórios e Favoritos ficam no menu lateral, agrupados por Receitas e despesas, Ativos e passivos e Patrimônio. O conteúdo do relatório usa a largura principal; os filtros permitem ajustar período, moedas de origem, contexto e regime quando aplicável.
- O eixo vertical do gráfico de Receitas x despesas usa marcas inteiras e arredondadas para cima, sem cortar a maior coluna. Os valores financeiros continuam em centavos inteiros.
- O total da lista de assinaturas ou recorrências soma apenas os valores atualmente exibidos, separadamente por moeda. Não converte moedas nem normaliza frequências distintas; a interface informa essa limitação.
- A moeda de saída dos relatórios é sempre a preferencial do perfil. O usuário escolhe por caixas de seleção quais moedas de origem entram na análise. Cada linha é convertida pela taxa implícita da transferência cambial concluída mais recente até o mês analisado; na ausência de taxa anterior, usa-se a primeira posterior disponível. Uma ponte por outra moeda suportada é aceita. Valores sem rota de conversão ficam fora do total e geram aviso explícito.
...
- Receitas x despesas aceita períodos predefinidos e personalizados, exibe valores inteiros no formato local e permite abrir os lançamentos do mês a partir das células da matriz.
- A evolução patrimonial recompõe, por mês, saldos de contas, avaliações patrimoniais, fotografias de investimentos e faturas de cartão ainda não pagas naquela data, tudo na moeda preferencial do perfil. Meses futuros seguem a projeção das contas e incluem movimentos ativos datados de hoje em diante.
...
- As matrizes consolidam categoria principal e usam o mesmo botão de abertura para categorias com ou sem subcategorias; valores diretos aparecem como `Sem subcategoria`. Os valores permanecem em centavos inteiros, são exibidos sem repetir o símbolo monetário e categorias e subcategorias são ordenadas pelo acumulado em ordem decrescente dentro de cada seção.
- Comparativos exibem o segundo período menos o primeiro; a variação percentual fica vazia quando o primeiro período é zero.
- Performance de ativos inicia pelo custo acumulado e apresenta rendimentos, valor atual, realizado, resultado da posição, resultado com rendimentos, resultado total e taxas total, mensal e anual. A visão de ativos considera somente posições ativas; a visão geral inclui liquidadas. Com histórico completo, o resultado total é `valor atual + resgates + rendimentos - aportes`, o lucro ou perda realizado compara resgates com o custo baixado, e a taxa anual é calculada pelos fluxos datados; a taxa mensal é a equivalente efetiva. Com histórico parcial, resultado total e retorno usam `valor atual - custo acumulado`, ficam identificados por `*` e as taxas mensal e anual permanecem vazias.
...
- Cotações de ações e fundos imobiliários são atualizadas em uma grade própria. O valor atual é sempre recalculado como quantidade multiplicada pelo valor unitário informado, com arredondamento para centavos inteiros, e a atualização cria a fotografia da posição na data corrente.
- A grade principal das posições usa um único cabeçalho matricial, ordena as classes pelo valor atual decrescente e mantém cada classe em um bloco visual independente. Detalhes unitários de ações e FIIs ficam na atualização da posição e na seção Cotações, não na listagem principal.
- Assinaturas de cartão não possuem quantidade fixa de parcelas. A primeira cobrança integra sua fatura real e as cinco competências seguintes são apresentadas como projeção; compras parceladas aparecem somente até a última parcela cadastrada. Cancelar uma compra é apresentado como exclusão e só é permitido enquanto suas faturas podem ser recalculadas com segurança.
- A lista principal de Recorrências apresenta regras automáticas de receitas e despesas. Uma seção separada reúne assinaturas das contas e dos cartões para revisão de custos; parcelas futuras continuam na central de cartões.
- O patrimônio apura cartões pela soma pendente das faturas não pagas de cada cartão ativo, sem considerar limite disponível nem cobranças apenas projetadas.
