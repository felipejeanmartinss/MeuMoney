...
export function CategoryGroupForm({ values }: { values: Values }) {
  const action = values.id ? updateCategoryGroup : createCategoryGroup;
  ...
  return (
    <form action={formAction} className="grid gap-5">
      ...
      <Field label="Nome do grupo" error={state.fieldErrors?.name?.[0]}>
        <input
      ...
      <SubmitButton pending={pending}>
        {values.id ? "Salvar grupo" : "Criar grupo"}
      </SubmitButton>
    </form>
  );
}
